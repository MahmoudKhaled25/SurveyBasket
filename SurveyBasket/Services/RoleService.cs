...
public class RoleService(RoleManager<ApplicationRole> roleManager,ApplicationDbContext context) : IRoleService
{
    ...
    public async Task<IEnumerable<RoleResponse>> GetAllRolesAsync(bool includeDisabled = false,CancellationToken cancellationToken = default) => await _roleManager.Roles
    ...
    {
        var roleIsExist = await _roleManager.Roles.AnyAsync(x => x.Id != id && x.Name == request.Name);
        if (roleIsExist)
        ...
        if(result.Succeeded)
        {
            ...
            await _context.RoleClaims.Where(x => x.RoleId == id && removedPermissions
                                     .Contains(x.ClaimValue))
                                     .ExecuteDeleteAsync();
